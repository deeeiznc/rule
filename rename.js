 ...
 * [blnx]   只保留高倍率
 * [clear]  清理乱名
 * [blpx]   如果用了上面的bl参数, 对保留标识后的名称分组排序 (未使用bl则本参数不生效)
...
const nameclear =
  /(套餐|到期|有效|剩余|版本|已用|过期|失联|测试|官方|网址|备用|群|TEST|客服|网站|获取|订阅|流量|机场|下次|官址|联系|邮箱|工单|学术|文档|USE|USED|TOTAL|EXPIRE|EMAIL)/i;
// prettier-ignore
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 是否保留固定格式(如 iplc/iepl/家宽/高倍率标识等)
    let ikey = "",
      ikeys = "";
    if (blgd) {
    ...
    // 查找地区映射
    const findKey = AMK.find(([mapKey]) => e.name.includes(mapKey));
    let firstName = "",
      nNames = "";

    if (nf) {
      firstName = FNAME; // 把前缀放最前
    } else {
      nNames = FNAME;    // 前缀放后面
    }
