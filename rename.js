...
// rurekey：在 rename.js 中用于提前替换一些常见关键字成地区名
const rurekey = {
  GB: /UK/g,
  "B-G-P": /BGP/g,
  ...
  if (clear || nx || blnx || key) {
    pro = pro.filter((res) => {
      ...
      const shouldKeep =
        ...
        // 如果 [key]=true，需满足 keya.test(...) 且含有 2|4|6|7 才保留
...
// 分组排序，用于将 specialRegex 匹配到的节点排后
function fampx(pro) {
  const wis = [];
  const wnout = [];
  ...
  // 按 specialRegex 出现顺序 + 字符排序
  wis.sort(
    (a, b) =>
      sps[wis.indexOf(a)] - sps[wis.indexOf(b)] ||
      a.name.localeCompare(b.name)
  );
  // 未匹配 specialRegex 的保持原顺序
