 ...
 * [in=flag]或 in=gq 识别国旗
 * [in=quan]识别英文全称
 *
 * [out=]   输出节点名可选参数: (cn或zh ，us或en ，gq或flag ，quan) 对应：(中文，英文缩写 ，国旗 ，英文全称) 
 *          默认中文 例如 [out=en] 或 out=us 输出英文缩写
 *** 分隔符参数
 *
 ...
 * [blnx]   只保留高倍率
 * [clear]  清理乱名
 * [blpx]   如果用了上面的bl参数, 对保留标识后的名称分组排序
 ...
 */
 ...
 const regexArray = [
   ...
   /IPLC/i, /IEPL/i, /核心/, /边缘/, /高级/, /标准/, /实验/, /商宽/, /家宽/, /游戏|game/i, /购物/, /专线/, /LB/, 
   /cloudflare/i, /\budp\b/i, /\bgpt\b/i, /udpn\b/
 ];
 // prettier-ignore
 const valueArray = [
 ...
 function operator(pro) {
   ...
   pro.forEach((e) => {
     let bktf = false; 
     let ens  = e.name;
     let matchedRegion = false;  // —— 用来判断是否在 rurekey 找到匹配
 
     // 1) 先尝试用 rurekey
     Object.keys(rurekey).forEach((ikey) => {
       if (rurekey[ikey].test(e.name)) {
         e.name = e.name.replace(rurekey[ikey], ikey);
         matchedRegion = true; // —— 标记已匹配到
     ...
     // 2) 如果未匹配到，用 fallbackRurekey 再试一次
     if (!matchedRegion) {
       Object.keys(fallbackRurekey).forEach((ikey) => {
         if (fallbackRurekey[ikey].test(e.name)) {
           e.name = e.name.replace(fallbackRurekey[ikey], ikey);
           matchedRegion = true; // 标记匹配到 fallback
           ...
           if (BLKEY) {
             ...
             BLKEYS.forEach((i) => {
               if (i.includes(">") && ens.includes(i.split(">")[0])) {
                 if (fallbackRurekey[ikey].test(i.split(">")[0])) {
                   e.name += " " + i.split(">")[0];
                 }
                 if (i.split(">")[1]) {
                   BLKEY_REPLACE = i.split(">")[1];
     ...
     // 如果依然没有匹配到 rurekey / fallbackRurekey，但用户允许保留 (nm)
     if (!matchedRegion) {
       if (nm) {
         // 保留原名加前缀
     ...
     // 拼接最终名字
     // —— 修改点：不再做节点顺序聚合编号，但是仍要生成想要的“地区 + 前缀”之类
     ...
     if (findKey?.[1]) {
       ...
       let finalArr = [firstName, usflag, nNames, findKeyValue, ikey, ikeys];
       finalArr = finalArr.filter((k) => k !== "");
       e.name = finalArr.join(FGF);
     }
   });
 ...
 //       found.items.push({
 //         ...cur,
 //         name: `${cur.name}${XHFGF}${found.count.toString().padStart(2, "0")}`,
 //       });
 //     } else {
 //       acc.push({
 //         name: cur.name,
 ...
 //   }, []);
 //   // 将分组后的 items 展平
 //   const flattened = n.flatMap((x) => x.items);
 ...
 // // 若只剩一个，则去掉 01
 // function oneP(e) {
 //   const grouped = e.reduce((acc, item) => {
 //     const baseName = item.name.replace(/[^A-Za-z0-9\u4E00-\u9FFF]+\d+$/, "");
 //     if (!acc[baseName]) acc[baseName] = [];
 //     acc[baseName].push(item);
 //     return acc;
 //   }, {});
 //   for (const k in grouped) {
 //     if (grouped[k].length === 1 && grouped[k][0].name.endsWith("01")) {
 //       grouped[k][0].name = grouped[k][0].name.replace(/01$/, "");
 //     }
 //   }
 //   return e;
 // }
 ...
 function fampx(pro) {
   ...
   withSpecial.sort(
     (a, b) =>
       sps[withSpecial.indexOf(a)] -
         sps[withSpecial.indexOf(b)] || a.name.localeCompare(b.name)
   );
