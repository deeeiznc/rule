...
const {
  nx = false, bl = false, nf = false, key = false,
  blgd = false, blpx = false, blnx = false,
  debug = false, clear = false, nm = false,
  flag: addflag = false
} = inArg;
...
const specialRegex = [
  /(\d\.)?\d+×/, /IPLC|IEPL|Kern|Edge|Pro|Std|Exp|Biz|Fam|Game|Buy|Zx|LB|Game/
];
const nameclear = /(套餐|到期|有效|剩余|版本|已用|过期|失联|测试|官方|网址|备用|群|TEST|客服|网站|获取|订阅|流量|机场|下次|官址|联系|邮箱|工单|学术|文档|USE|USED|TOTAL|EXPIRE|EMAIL|TRAFFIC)/i;
...
const rurekey = {
  ...
  泰国: /泰國|曼谷/g,
  法国: /巴黎/g,
  G: /\d\s?GB/gi, Esnc: /esnc/gi
};
let GetK = false, AMK = [];
const ObjKA = i => { GetK = true; AMK = Object.entries(i); };

function operator(pro) {
  const Allmap = {}, outList = getList(outputName);
  const inputList = inname ? [getList(inname)] : [ZH, FG, QC, EN];
  for (const arr of inputList) arr.forEach((v, i) => Allmap[v] = outList[i]);
  if (!GetK) ObjKA(Allmap);
  if (clear || nx || blnx || key)
    pro = pro.filter(r => !(clear && nameclear.test(r.name)) &&
      !(nx && namenx.test(r.name)) &&
      !(blnx && !nameblnx.test(r.name)) &&
      !(key && !(keya.test(r.name) && /2|4|6|7/i.test(r.name))));
  ...
  for (const e of pro) {
        ...
        if (BLKEY) {
          ...
          for (const k of BLKEYS) {
            ...
            if (parts[1] && ens.includes(parts[0])) {
              if (reg.test(parts[0])) e.name += " " + parts[0];
              retainKey = parts[1];
    ...
    e["block-quic"] = (blockquic === "on" || blockquic === "off") ? blockquic : (delete e["block-quic"], undefined);
    if (!bktf && BLKEY)
      for (const k of BLKEYS) {
        const parts = k.split(">");
        if (parts[1] && e.name.includes(parts[0])) retainKey = parts[1];
      }
    let ikey = "", ikeys = "";
    if (blgd)
      for (let i = 0; i < regexArray.length; i++)
        regexArray[i].test(e.name) && (ikeys = valueArray[i]);
    if (bl) {
    ...
    if (findKey && findKey[1]) {
      ...
      const findKeyValue = findKey[1],
        idx = outList.indexOf(findKeyValue);
      if (addflag && idx !== -1)
        usflag = FG[idx] === "🇹🇼" ? "🇨🇳" : FG[idx];
      e.name = [firstName, usflag, nNames, findKeyValue, retainKey, ikey, ikeys].filter(Boolean).join(FGF);
    } else {
      const m = e.name.match(/[澳德港日新坡美台韩俄泰法]/);
      if (m) {
        const map = { "澳": "AU", "德": "DE", "港": "HK", "日": "JP", "坡": "SG", "美": "US", "台": "TW", "韩": "KR", "俄": "RU", "泰": "TH", "法": "FR" },
          findKeyValue = map[m[0]];
        let usflag = "";
