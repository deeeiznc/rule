...
function operator(pro) {
  ...
  for (const e of pro) {
    ...
    let keyVal;
    if (findKey?.[1]) keyVal = findKey[1];
    else keyVal = (const m = e.name.match(/[澳德港日新坡美台韩俄泰法]/)) ?{ "澳": "AU", "德": "DE", "港": "HK", "日": "JP", "坡": "SG", "美": "US", "台": "TW", "韩": "KR", "俄": "RU", "泰": "TH", "法": "FR" } [m[0]] : null;
    if (keyVal) {
      const idx = outList.indexOf(keyVal),
        usflag = addflag && idx !== -1 ? (FG[idx] === "🇹🇼" ? "🇨🇳" : FG[idx]) : "";
      e.name = [firstName, usflag, nNames, keyVal, retainKey, ikey, ikeys].filter(Boolean).join(FGF);
