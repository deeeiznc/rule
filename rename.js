...
const specialRegex = [/(\d\.)?\d+×/, /IPLC|IEPL|Kern|Edge|Pro|Std|Exp|Biz|Fam|Game|Buy|Zx|LB|Game/],
  nameclear = /(套餐|到期|有效|剩余|版本|已用|过期|失联|测试|官方|网址|备用|群|TEST|客服|网站|获取|订阅|流量|机场|下次|官址|联系|邮箱|工单|学术|文档|USE|USED|TOTAL|EXPIRE|EMAIL|TRAFFIC)/i,
...
const getList = a => a === "us" ? EN : a === "gq" ? FG : a === "quan" ? QC : ZH;

function operator(pro) {
  const outList = getList(outputName),
    inputList = inname ? [getList(inname)] : [ZH, FG, QC, EN],
    Allmap = {};
  inputList.forEach(arr => arr.forEach((v, i) => Allmap[v] = outList[i]));
  if (!GetK) ObjKA(Allmap);
  if (clear || nx || blnx || key)
    pro = pro.filter(r =>
      (!clear || !nameclear.test(r.name)) &&
  ...
  for (const e of pro) {
    ...
    if (!bktf && BLKEY)
      for (const k of BLKEYS) { const parts = k.split(">"); parts[1] && e.name.includes(parts[0]) && (retainKey = parts[1]); }
    let ikey = "", ikeys = "";
    if (blgd)
    ...
    if (findKey && findKey[1]) {
      let usflag = "";
      const findKeyValue = findKey[1], idx = outList.indexOf(findKeyValue);
      addflag && idx !== -1 && (usflag = FG[idx] === "🇹🇼" ? "🇨🇳" : FG[idx]);
      ...
      if (m) {
        const mapObj = { "澳": "AU", "德": "DE", "港": "HK", "日": "JP", "坡": "SG", "美": "US", "台": "TW", "韩": "KR", "俄": "RU", "泰": "TH", "法": "FR" },
          findKeyValue = mapObj[m[0]],
