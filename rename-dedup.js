...
function operator(pro) {
  const allMap = {};
  (inName ? [inName] : [cc, zh, en, flag]).forEach(arr => arr.forEach((v, i) => allMap[v] = outName[i]));
  ...
  for (const e of pro) {
    ...
    if (keyVal) {
      const idx = outName.indexOf(keyVal);
